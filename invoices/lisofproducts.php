...
// إضافة: الحصول على AgeGroupID من الجلسة إذا كان موجودًا
$ageGroupCondition = "";
if (isset($_SESSION['PatientAgeGroupID']) && $_SESSION['PatientAgeGroupID'] !== null) {
  $ageGroupID = $_SESSION['PatientAgeGroupID'];
  $ageGroupCondition = " AND AgeGroupID = '{$ageGroupID}'";
}

$sql = "SELECT * FROM `tblservices` p WHERE  
                        SKU LIKE '%$search_value%' 
                       OR Services LIKE '%$search_value%'
                       OR Description LIKE '%$search_value%'" . $ageGroupCondition;
