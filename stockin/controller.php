...
function doQty()
{
	...
	if (isset($_POST['save'])) {
		...
		for ($i = 0; $i < $qty; $i++) {
			...
			$sql = "SELECT * FROM tblstocks WHERE ProductID='" . trim($_POST['ProductID']) . "'";
			$mydb->setQuery($sql);
			$cur = $mydb->executeQuery();
			$maxrow = $mydb->num_rows($cur);
			...
			if ($maxrow > 0) {
				...
				$stocks = new Stocks();
				$stocks->ProductID		= trim($_POST['ProductID']);
				$stocks->Products		= trim($res->Products);
				// $stocks->Brand			= trim($res->Brand); 
				$stocks->Brand			= trim('');
				$stocks->Category		= trim($res->Category);
				$stocks->Description	= $res->Description;
				$stocks->Stocks	= 1;
				$stocks->create();
			}
	...
	if ($qty > 0) {
		...
		if ($maxrow > 0) {
			...
			if ($qty > $res->r) {
				...
				message("$qty Product has been sold!", "success");
				redirect("index.php");
			}
		}
	} else {
		message("Input the correct amount!", "error");
		redirect("index.php?view=sold&id=" . $proid);
	}
}
